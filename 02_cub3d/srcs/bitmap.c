...
/*   Updated: 2020/12/25 14:40:18 by floogman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	write_pixels(t_data *s, int fd)
{
	...
	while (y >= 0)
	{
		...
		while (x < s->win.w)
		{
			clr[2] = (unsigned char)get_r(get_pixel(s->img, x, y));
			clr[1] = (unsigned char)get_g(get_pixel(s->img, x, y));
			clr[0] = (unsigned char)get_b(get_pixel(s->img, x, y));
			write(fd, clr, 3);
			x++;
		}
		y--;
